-- Phase 4D exact output prompt contract. Historical runs and applied migrations remain untouched.
-- Replace only the bounded reservation function with the same authorization and lifecycle guards.
begin;

alter table public.generation_runs add constraint generation_runs_exact_output_projection_check
  check (prompt_version <> 'phase4d-exact-output/1.0.0' or projection_hash is not null);
...
begin
  if p_employee is null or p_input is null or jsonb_typeof(p_input) <> 'object'
     ...
     or p_prompt_version is distinct from 'phase4d-exact-output/1.0.0'
     or p_template_hash is distinct from 'f8ce3cc932b45cec0c0fe132b18707f35eb0809d54e5af1dd2707aa960998401'
