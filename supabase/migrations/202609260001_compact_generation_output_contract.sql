-- Phase 4D compact exact output prompt contract. Historical runs and applied migrations remain untouched.
-- Replace only the bounded reservation function with the same authorization and lifecycle guards.
begin;

alter table public.generation_runs add constraint generation_runs_compact_output_projection_check
  check (prompt_version <> 'phase4d-compact-exact-output/1.0.0' or projection_hash is not null);
...
begin
  if p_employee is null or p_input is null or jsonb_typeof(p_input) <> 'object'
     ...
     or p_prompt_version is distinct from 'phase4d-compact-exact-output/1.0.0'
     or p_template_hash is distinct from 'ed734c3b6d71d456bbd5114c63cc945ae7147f07d4a3a41e72ec284abea6c7c1'
