-- Phase 4D: permit Groq as an alternative active provider without altering historical runs.
-- Existing RBAC, RLS, frozen-context validation, idempotency, and lifecycle rules remain unchanged.
begin;

alter table public.generation_runs drop constraint generation_runs_provider_check;
alter table public.generation_runs add constraint generation_runs_provider_check
...
create or replace function public.reserve_generation_run(
...
-- Preserve the existing function owner, SECURITY DEFINER, fixed search_path,
-- and grants; CREATE OR REPLACE does not create a new function identity.
commit;
